...
# $Id: GSASIIdataGUI.py 3485 2018-07-18 20:46:38Z vondreele $
########### SVN repository information ###################
'''
...
try:
    ...
    pass
import GSASIIpath
GSASIIpath.SetVersionNumber("$Revision: 3485 $")
...
# </ Anton Gagin    
        item = parent.Append(wx.ID_ANY, 'Open .lst file', '')
        self.Bind(wx.EVT_MENU, self.OnLstRead, id=item.GetId())
        item = parent.Append(wx.ID_ANY, 'Compare standard and Bayesian fits', '')
        self.Bind(wx.EVT_MENU, self.OnCompareRead, id=item.GetId())        
# Anton Gagin />		
...
# </ Anton Gagin        
    def OnLstRead(self,event):
        ...
        fname = ospath.splitext(self.GSASprojectfile)[0]+'.lst'
        if(os.path.isfile(fname)):
            webbrowser.open(fname)
        else: 
            print('No such file yet!')
			
    def OnCompareRead(self,event):
        import webbrowser
        import os.path as ospath
        fname = ospath.splitext(self.GSASprojectfile)[0]+'-compare.lst'
        if(os.path.isfile(fname)):
            webbrowser.open(fname)
        else: 
            print('No such file yet!')        
            ...
            try:
                if dlg.ShowModal() == wx.ID_OK:
                    ...
                    if Id:
                        ...
                        self.GPXtree.SetItemPyData(self.GPXtree.AppendItem(Id,text='Background'),[['chebyschev',True,3,1.0,0.0,0.0],
                            {'nDebye':0,'debyeTerms':[],'nPeaks':0,'peaksList':[]}])
                        self.GPXtree.SetItemPyData(self.GPXtree.AppendItem(Id,text='Instrument Parameters'),Inst)
...
#            help='Reorganize variables where variables change')
        ...
        self.ModelEdit.Append(G2G.wxID_MODELFITALL,'Sequential fit','Sequential fit of model parameters to all SASD data')
...
#            wx.CallAfter(UpdateControls,G2frame,data)         
        ...
        MargMultSizer.Add(wx.StaticText(G2frame.dataWindow,label=' Number of knots E_mu: '),0,WACV)
        KnotsNumC = wx.TextCtrl(G2frame.dataWindow,-1,value=data['corrParam E_mu'],style=wx.TE_PROCESS_ENTER)
        KnotsNumC.SetValue(data['corrParam E_mu'])
        ...
        MargMultSizer.Add(wx.StaticText(G2frame.dataWindow,label=' Prior factor k_mu: '),0,WACV)
        MargKMu = wx.TextCtrl(G2frame.dataWindow,-1,value=data['corrParam k_mu'],style=wx.TE_PROCESS_ENTER)
        MargKMu.SetValue(data['corrParam k_mu'])
        MargKMu.Bind(wx.EVT_TEXT_ENTER, OnMargKMu)
        MargKMu.Bind(wx.EVT_KILL_FOCUS, OnMargKMu)
        MargMultSizer.Add(MargKMu,0,WACV)   
        
        OptKMu = wx.CheckBox(G2frame.dataWindow,-1,label=' Estimate optimal k_mu?')
...
#            wx.CallAfter(UpdateControls,G2frame,data)         
        ...
        MargAddSizer.Add(wx.StaticText(G2frame.dataWindow,label=' Number of knots E_beta: '),0,WACV)
        KnotsNumB = wx.TextCtrl(G2frame.dataWindow,-1,value=data['corrParam E_beta'],style=wx.TE_PROCESS_ENTER)
        KnotsNumB.SetValue(data['corrParam E_beta'])
        ...
        MargAddSizer.Add(wx.StaticText(G2frame.dataWindow,label=' Prior factor k_beta: '),0,WACV)
        MargKBeta = wx.TextCtrl(G2frame.dataWindow,-1,value=data['corrParam k_beta'],style=wx.TE_PROCESS_ENTER)
        MargKBeta.SetValue(data['corrParam k_beta'])
        ...
        OptKBeta = wx.CheckBox(G2frame.dataWindow,-1,label=' Estimate optimal k_beta?')
...
# additional controls in main Controls data tree entry: number of blocks s
        ...
        MargNumBlocksSizer.Add(wx.StaticText(G2frame.dataWindow,label=' Number of blocks s: '),0,WACV)
        BlocksNum = wx.TextCtrl(G2frame.dataWindow,-1,value=data['corrParam num blocks s'],style=wx.TE_PROCESS_ENTER)
        BlocksNum.SetValue(data['corrParam num blocks s'])
...
#            wx.CallAfter(UpdateControls,G2frame,data)     
        ...
        MargShapeSizer.Add(wx.StaticText(G2frame.dataWindow,label=' Correlation legth l_delta: '),0,WACV)
        LDel = wx.TextCtrl(G2frame.dataWindow,-1,value=data['corrParam l_delta'],style=wx.TE_PROCESS_ENTER)
        LDel.SetValue(data['corrParam l_delta'])
        LDel.Bind(wx.EVT_TEXT_ENTER, OnLDel)
        LDel.Bind(wx.EVT_KILL_FOCUS, OnLDel)
        MargShapeSizer.Add(LDel,0,WACV)
        
        MargShapeSizer.Add(wx.StaticText(G2frame.dataWindow,label=' OR estimate it as FWHM /'),0,WACV)
        FWHMDiv = wx.TextCtrl(G2frame.dataWindow,-1,value=data['corrParam FWHMDivN'],style=wx.TE_PROCESS_ENTER)
        FWHMDiv.SetValue(data['corrParam FWHMDivN'])
        FWHMDiv.Bind(wx.EVT_TEXT_ENTER, OnFWHMDiv)
        FWHMDiv.Bind(wx.EVT_KILL_FOCUS, OnFWHMDiv)
        MargShapeSizer.Add(FWHMDiv,0,WACV)
        
        MargShapeSizer.Add(wx.StaticText(G2frame.dataWindow,label=' Stdev sigma_delta: '),0,WACV)
        SigDel = wx.TextCtrl(G2frame.dataWindow,-1,value=data['corrParam sigma_delta'],style=wx.TE_PROCESS_ENTER)
        SigDel.SetValue(data['corrParam sigma_delta'])
        SigDel.Bind(wx.EVT_TEXT_ENTER, OnSigDel)
        SigDel.Bind(wx.EVT_KILL_FOCUS, OnSigDel)
        MargShapeSizer.Add(SigDel,0,WACV)

        MargShapeSizer.Add(wx.StaticText(G2frame.dataWindow,label=' OR estimate it as l_delta /'),0,WACV)
        LDelDiv = wx.TextCtrl(G2frame.dataWindow,-1,value=data['corrParam l_deltaDivN'],style=wx.TE_PROCESS_ENTER)
        LDelDiv.SetValue(data['corrParam l_deltaDivN'])
...
# additional controls in main Controls data tree entry: iterative procedure
        ...
        DoIter = wx.CheckBox(G2frame.dataWindow,-1,label=' Iterative (may cause overfitting)')
...
# MCMC controls   
        ...
        doMCMC = wx.CheckBox(G2frame.dataWindow,-1,label=' run sampler for MCMC?')
...
# MCMC controls 2
        ...
        MargMCMCControlsSizer.Add(wx.StaticText(G2frame.dataWindow,label=' Number of Goodman&Weare walkers: '),0,WACV)
        NWalkers = wx.TextCtrl(G2frame.dataWindow,-1,value=data['nwalkers'],style=wx.TE_PROCESS_ENTER)
        NWalkers.SetValue(data['nwalkers'])
        ...
        MargMCMCControlsSizer.Add(wx.StaticText(G2frame.dataWindow,label=' Number of steps: '),0,WACV)
        nIterMCMC = wx.TextCtrl(G2frame.dataWindow,-1,value=data['nIterMCMC'],style=wx.TE_PROCESS_ENTER)
        nIterMCMC.SetValue(data['nIterMCMC'])
...
# </ Anton Gagin               
   # additional controls in main Controls data tree entry 
    ...
    mainSizer.Add(wx.StaticLine(G2frame.dataWindow, -1, (1, 1), (500,1)))
    mainSizer.Add((10,10),0)    
    mainSizer.Add(wx.StaticText(G2frame.dataWindow,label=' Systematic Error Marginalization Controls:'),0,WACV)    
    mainSizer.Add((10,10),0)
    mainSizer.Add(wx.StaticText(G2frame.dataWindow,label=' Multiplicative Correction:'),0,WACV)    
    mainSizer.Add(MargMultSizer())
    mainSizer.Add((10,10),0)
    mainSizer.Add(wx.StaticText(G2frame.dataWindow,label=' Additive Correction:'),0,WACV)    
    mainSizer.Add(MargAddSizer())
    mainSizer.Add((10,10),0)    
    mainSizer.Add(wx.StaticText(G2frame.dataWindow,label=' Profile Peak-Shape Correction:'),0,WACV)    
    ...
    mainSizer.Add((10,10),0)
    mainSizer.Add(wx.StaticText(G2frame.dataWindow,label=' MCMC ensemble sampler:'),0,WACV)    
